	...
	<div class="primary_block row">
		...
		<div class="pb-center-column col-xs-12 col-sm-4">
			...
			<p id="product_reference"{if empty($product->reference) || !$product->reference} style="display: none;"{/if}>
				<label>{l s='#'} </label>
				<strong><span class="editable" itemprop="sku"{if !empty($product->reference) && $product->reference} content="{$product->reference}"{/if}>{if !isset($groups)}{$product->reference|escape:'html':'UTF-8'}{/if}</span></strong>
			</p>
			...
			<p id="product_condition">
				...
				{if $product->condition == 'new'}
					...
					<span class="editable">{l s='Nuevo'}</span>
...
{if (isset($quantity_discounts) && count($quantity_discounts) > 0)}
		...
		{if isset($features) && $features}
			<!-- Data sheet -->
			<section class="page-product-box">
				<h3 class="page-product-heading" id="datasheet">{l s='Data sheet'}</h3>
